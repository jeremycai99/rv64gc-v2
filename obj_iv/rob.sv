...
(
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            begin
                logic [8:0] nh_sum = {1'b0, head_r} + {6'd0, commit_count};
                logic [7:0] nh = (nh_sum >= 9'(ROB_DEPTH)) ?
                ...
                begin
                    logic [7:0] max_valid = count_r - {5'd0, commit_count};
                    logic [7:0] raw_count;
            ...
            begin
                logic [8:0] nt_sum = {1'b0, tail_r} + {6'd0, alloc_count};
                logic [7:0] nt = (nt_sum >= 9'(ROB_DEPTH)) ?
                                           nt_sum[7:0] - ROB_DEPTH_U8 : nt_sum[7:0];

                for (int i = 0; i < PIPE_WIDTH; i++) begin
                    if (alloc_count > i[2:0]) begin
                        logic [8:0] ai_sum = {1'b0, tail_r} + 9'(i);
                        logic [7:0] ai = (ai_sum >= 9'(ROB_DEPTH))
