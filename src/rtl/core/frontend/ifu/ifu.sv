...
(
    ...
    input  wire [63:0]                   redirect_pc_i,

    input  wire                          frontend_hold_i,
    input  wire                          translation_stall_i,
    input  wire                          packet_buf_full_i,
    input  wire                          icq_full_i,
    input  wire                          ftq_enq_ready_i,
    input  wire                          remainder_valid_i,
    input  wire                          same_owner_continue_i,
    input  wire                          req_redirect_i,
    input  wire                          duplicate_suppressed_i,
    input  wire [63:0]                   duplicate_next_pc_i,
    input  wire                          pc_consumed_i,
    input  wire                          will_emit_i,

    input  wire [FTQ_IDX_BITS-1:0]       req_owner_idx_i,
    input  wire [FTQ_EPOCH_BITS-1:0]     req_owner_epoch_i,
    input  wire [FTQ_ALLOC_TAG_BITS-1:0] req_owner_tag_i,
    ...
    input  wire [FTQ_EPOCH_BITS-1:0]     ftq_current_epoch_i,
    input  wire [FTQ_IDX_BITS:0]         ftq_count_ifu_to_wb_i,
    input  wire [FTQ_IDX_BITS:0]         ftq_count_alloc_to_ifu_i,
    ...
    input  wire                          packet_enq_i,

    output reg                          f1_valid_o,
    output reg [63:0]                   f1_pc_o,
    output reg [63:0]                   req_pc_o,
    output reg [63:0]                   req_block_pc_o,
    output reg                          ftq_need_alloc_o,
    output reg                          ifu_req_valid_o,
    output reg                          ifu_req_ready_o,
    output reg                          ifu_req_fire_o,
    output reg                          ftq_enq_valid_o,
    output reg                          ftq_ifu_req_pop_valid_o,
    output reg                          ftq_delivery_push_valid_o,
    output reg                          ftq_ifu_pop_valid_o,
    output reg                          ic_req_valid_o,
    output reg [63:0]                   ic_req_addr_o,
    output reg                          fe_stall_o,
    output reg                          consumed_remainder_o,
    output reg                          owner_live_o,
    output reg                          redirect_without_owner_successor_o,
    output reg                          owner_completion_candidate_o,
    output reg                          owner_delivery_push_o,

    output reg                          work_valid_o,
    output reg [63:0]                   work_pc_o,
    output reg                          work_ftq_valid_o,
    output reg [FTQ_IDX_BITS-1:0]       work_ftq_idx_o,
    output reg [FTQ_EPOCH_BITS-1:0]     work_ftq_epoch_o,
    output reg [FTQ_ALLOC_TAG_BITS-1:0] work_ftq_alloc_tag_o,
    output ftq_entry_t                    work_ftq_entry_o,
    output reg                          work_line_valid_o,
    output reg [63:LINE_BITS]           work_line_addr_o,
    output reg                          work_owner_delivered_o,

    output reg                          work_redirect_o,
    output reg                          work_redirect_next_owner_match_o,
    output reg                          work_redirect_enq_owner_o,
    output reg                          work_redirect_keep_owner_o,
    output reg                          work_take_ftq_next_owner_o,
    output reg                          work_take_request_owner_o,
    output reg                          work_take_remainder_request_owner_o,
    output reg                          work_same_owner_advance_o,

    output reg                          runahead_req_valid_o,
    output reg                          runahead_req_fire_o,
    output reg                          runahead_cancel_next_o,
    output reg                          runahead_pending_o,
    output reg [63:0]                   runahead_pending_pc_o,
    output reg [FTQ_IDX_BITS-1:0]       runahead_pending_idx_o,
    output reg [FTQ_EPOCH_BITS-1:0]     runahead_pending_epoch_o,
    output reg [FTQ_ALLOC_TAG_BITS-1:0] runahead_pending_tag_o,
    output reg                          runahead_redirect_match_o,
    output reg                          runahead_duplicate_alloc_blocked_o,
    output reg                          runahead_depth_gt1_o
);
    ...
    // VIPT F0/F1: the translation stall no longer gates the F0 advance — F0 is
    // feed-forward and ITLB misses are handled by F1 redirect-on-miss replay. Removing
    // it from fe_stall_c breaks the comb loop
    // (translation_stall -> fe_stall -> req_pc -> itlb_va -> itlb_hit -> translation_stall).
    // NOTE: the translation_stall_i input port (line 17) is now orphaned (kept for the
    // profiler's instr_translation_stall observation); safe to retire in a follow-up.
