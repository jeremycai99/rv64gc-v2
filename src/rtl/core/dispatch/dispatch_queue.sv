...
(
    ...
    always_comb begin : deq_output_comb
        ...
        // Suppress dequeue on flush — dispatching stale pre-flush entries
        // into the IQ while the ROB is being cleared creates orphaned IQ
        // entries whose ROB indices are invalid.
        if (flush_valid)
            deq_count = 3'd0;
        else
