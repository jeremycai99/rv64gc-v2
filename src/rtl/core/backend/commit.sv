...
(
    ...
    input  wire [ROB_IDX_BITS-1:0] head_idx,
    input  wire [PIPE_WIDTH-1:0]   head_valid,
    input  wire [PIPE_WIDTH-1:0]   head_ready,
    input  wire [63:0]             head_pc [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_has_exception,
    input  wire [3:0]              head_exc_code [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_is_branch,
    input  wire [2:0]              head_bpu_type [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_is_store,
    input  wire [PIPE_WIDTH-1:0]   head_is_load,
    input  wire [PIPE_WIDTH-1:0]   head_is_csr,
    input  wire [PIPE_WIDTH-1:0]   head_is_fence,
    input  wire [PIPE_WIDTH-1:0]   head_is_fence_i,
    input  wire [PIPE_WIDTH-1:0]   head_is_mret,
    input  wire [PIPE_WIDTH-1:0]   head_is_sret,
    input  wire [PIPE_WIDTH-1:0]   head_is_sfence_vma,
    input  wire [PIPE_WIDTH-1:0]   head_is_ecall,
    input  wire [PIPE_WIDTH-1:0]   head_is_wfi,
    input  wire [PIPE_WIDTH-1:0]   head_is_fused,
    input  wire                    fence_i_ready,
    input  wire                    sfence_vma_ready,
    input  wire [PIPE_WIDTH-1:0]   head_branch_taken,
    input  wire [63:0]             head_branch_target [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_branch_mispredict,
    input  wire [11:0]             head_csr_addr [0:PIPE_WIDTH-1],
    input  wire [63:0]             head_csr_wdata [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_csr_we,
    input  wire [1:0]              head_csr_op [0:PIPE_WIDTH-1],

    // Rename buffer data (for free list release)
    input  wire [PHYS_REG_BITS-1:0] head_pdst [0:PIPE_WIDTH-1],
    input  wire [PHYS_REG_BITS-1:0] head_old_pdst [0:PIPE_WIDTH-1],
    input  wire [4:0]              head_rd_arch [0:PIPE_WIDTH-1],
    input  wire [PIPE_WIDTH-1:0]   head_rd_valid,

    // Checkpoint release info
    input  wire [PIPE_WIDTH-1:0]              head_uses_checkpoint,
    input  wire [CHECKPOINT_BITS-1:0]         head_checkpoint_id [0:PIPE_WIDTH-1],
    input  wire [4:0]                         head_bp_ras_tos [0:PIPE_WIDTH-1],
    input  wire [63:0]                        head_bp_ras_top [0:PIPE_WIDTH-1],
    input  wire [1:0]                         head_bp_ras_op [0:PIPE_WIDTH-1],
    ...
    input  wire [63:0]             sepc,
    input  wire [1:0]              priv_mode,     // current privilege level
