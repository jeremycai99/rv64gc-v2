...
BOTTLENECK_COUNTERS = [
    ...
    ("xs_bottleneck_dep_load_consumer_alu", "dependency/wakeup", "load-produced source waits consumed by ALU uops"),
    ("xs_bottleneck_dep_load_consumer_bru", "dependency/wakeup", "load-produced source waits consumed by branch uops"),
    ("xs_bottleneck_dep_load_consumer_load_addr", "dependency/wakeup", "load-produced source waits consumed by load-address uops"),
    ("xs_bottleneck_dep_load_consumer_store_addr", "dependency/wakeup", "load-produced source waits consumed by store-address uops"),
    ("xs_bottleneck_dep_load_consumer_store_data", "dependency/wakeup", "load-produced source waits consumed by store-data uops"),
    ("xs_bottleneck_dep_load_consumer_other", "dependency/wakeup", "load-produced source waits consumed by other uops"),
    ("xs_bottleneck_dep_load_wait_issue_same_cycle", "dependency/wakeup", "load-produced sources waiting in the same cycle their producer issued"),
    ("xs_bottleneck_dep_load_wait_not_issued", "dependency/wakeup", "load-produced sources whose producer had not issued yet"),
    ("xs_bottleneck_dep_load_wait_issued_not_wb", "dependency/wakeup", "load-produced sources whose producer issued but had not written back"),
    ("xs_bottleneck_dep_load_wait_done_stale", "dependency/wakeup invariant", "load-produced sources still not ready after producer writeback"),
    ("xs_bottleneck_dep_load_wait_state_unknown", "dependency/wakeup", "load-produced sources with unknown producer lifecycle state"),
    ...
    ("xs_bottleneck_dep_branch_consumer_alu", "dependency/wakeup", "branch uop source waits on ALU-produced operands"),
    ("xs_bottleneck_dep_branch_consumer_load", "dependency/wakeup", "branch uop source waits on load-produced operands"),
    ("xs_bottleneck_dep_branch_consumer_store", "dependency/wakeup", "branch uop source waits on store-side operands"),
    ("xs_bottleneck_dep_branch_consumer_csr", "dependency/wakeup", "branch uop source waits on CSR-produced operands"),
    ("xs_bottleneck_dep_branch_consumer_mul", "dependency/wakeup", "branch uop source waits on multiply-produced operands"),
    ("xs_bottleneck_dep_branch_consumer_div", "dependency/wakeup", "branch uop source waits on divide-produced operands"),
    ("xs_bottleneck_dep_branch_consumer_unknown", "dependency/wakeup", "branch uop source waits on unknown producer classes"),
    ...
    ("xs_bottleneck_load_wakeup_candidate", "dependency/wakeup", "resident entries made eligible by a load wakeup this cycle"),
    ("xs_bottleneck_load_wakeup_selected", "dependency/wakeup", "load-woken resident entries selected this cycle"),
    ("xs_bottleneck_load_wakeup_missed", "dependency/wakeup", "load-woken resident entries not selected this cycle"),
    ("xs_bottleneck_load_wakeup_missed_port_busy", "issue queue", "load-woken entries missed because issue capacity was occupied"),
    ("xs_bottleneck_load_wakeup_missed_older_ready", "issue queue", "load-woken entries missed behind other ready work"),
    ("xs_bottleneck_load_wakeup_missed_fu_blocked", "issue queue", "load-woken entries missed by FU or port restriction"),
    ("xs_bottleneck_load_wakeup_missed_suppress_flush", "issue queue", "load-woken entries missed during suppress or flush"),
    ("xs_bottleneck_load_wakeup_missed_other", "issue queue", "load-woken entries missed for uncategorized reasons"),
    ...
    ("xs_bottleneck_rob_head_not_ready_load", "commit", "ROB head valid-not-ready cycles with a load at head"),
    ("xs_bottleneck_rob_head_not_ready_store", "commit", "ROB head valid-not-ready cycles with a store at head"),
    ("xs_bottleneck_rob_head_not_ready_branch", "commit", "ROB head valid-not-ready cycles with a branch at head"),
    ("xs_bottleneck_rob_head_not_ready_serial", "commit", "ROB head valid-not-ready cycles with serial work at head"),
    ("xs_bottleneck_rob_head_not_ready_other", "commit", "ROB head valid-not-ready cycles with other work at head"),
    ("xs_bottleneck_lsu_load_reissue_total", "LSU", "load issues that replaced an already tracked pending load"),
    ("xs_bottleneck_lsu_load_issue_candidate_total", "LSU", "total ready load issue candidate slots before LSU suppression"),
    ("xs_bottleneck_lsu_load_issue_fire_total", "LSU", "total load issue slots that fired after LSU suppression"),
    ("xs_bottleneck_lsu_load_issue_lost_slots", "LSU", "load issue candidate slots that did not fire"),
    ("xs_bottleneck_lsu_load_issue_no_fire_cycles", "LSU", "cycles with at least one load candidate but no load issue"),
    ("xs_bottleneck_lsu_load_issue_dual_candidate_cycles", "LSU", "cycles with two ready load candidates"),
    ("xs_bottleneck_lsu_load_issue_dual_fire_cycles", "LSU", "cycles with both load issue ports firing"),
    ("xs_bottleneck_lsu_load_issue_dual_candidate_single_fire_cycles", "LSU", "cycles with two load candidates but only one load issue"),
    ("xs_bottleneck_lsu_load_issue_suppress_total", "LSU", "load issue candidates suppressed by LSU or memory ordering"),
    ("xs_bottleneck_lsu_load_issue_suppress_ld0", "LSU", "load port 0 candidates suppressed by LSU or memory ordering"),
    ("xs_bottleneck_lsu_load_issue_suppress_ld1", "LSU", "load port 1 candidates suppressed by LSU or memory ordering"),
    ("xs_bottleneck_lsu_load_issue_store_order_ld0", "LSU", "load port 0 candidates blocked by older store address or forwarding order"),
    ("xs_bottleneck_lsu_load_issue_store_order_ld1", "LSU", "load port 1 candidates blocked by older store address or forwarding order"),
    ("xs_bottleneck_lsu_load_issue_store_order_same_cycle_sta_ld0", "LSU", "load port 0 store-order blocks where STA issued in the same cycle"),
    ("xs_bottleneck_lsu_load_issue_store_order_same_cycle_sta_ld1", "LSU", "load port 1 store-order blocks where STA issued in the same cycle"),
    ("xs_bottleneck_lsu_load_issue_p1_retry_captures", "LSU", "load port 1 retry captures after suppression or conflict"),
    ...
    ("xs_bottleneck_lsu_store_queue_wait", "LSU", "load cycles blocked by store queue ordering or forwarding wait"),
    ("xs_bottleneck_lsu_store_fwd_addr_missing", "LSU", "store-forward wait cycles attributed to older store address visibility"),
    ("xs_bottleneck_lsu_store_fwd_data_missing", "LSU", "store-forward wait cycles attributed to missing older store data"),
    ("xs_bottleneck_lsu_store_fwd_path_busy", "LSU", "store-forward wait cycles attributed to forwarding writeback path busy"),
    ("xs_bottleneck_lsu_store_fwd_partial", "LSU", "store-forward wait cycles attributed to partial or unaligned forwarding"),
    ("xs_bottleneck_lsu_store_fwd_spill_hold", "LSU", "store-forward wait cycles attributed to port-1 spill or hold state"),
    ("xs_bottleneck_lsu_store_fwd_backlog", "LSU", "cycles with store commit requests active during store-forward analysis"),
    ("xs_bottleneck_lsu_store_fwd_unknown", "LSU", "store-forward wait cycles with uncategorized cause"),
    ("xs_bottleneck_lsu_sq_addr_unknown_p0", "LSU", "port 0 store queue wait cycles caused by older stores with unresolved address"),
    ("xs_bottleneck_lsu_sq_addr_unknown_p1", "LSU", "port 1 store queue wait cycles caused by older stores with unresolved address"),
    ("xs_bottleneck_lsu_sq_data_missing_p0", "LSU", "port 0 store queue wait cycles caused by known older store data not ready"),
    ("xs_bottleneck_lsu_sq_data_missing_p1", "LSU", "port 1 store queue wait cycles caused by known older store data not ready"),
    ("xs_bottleneck_lsu_spec_past_addr_unknown_total", "LSU", "loads issued speculatively past older stores with unresolved addresses"),
    ("xs_bottleneck_lsu_spec_past_addr_unknown_p0", "LSU", "port 0 loads issued speculatively past unresolved older store addresses"),
    ("xs_bottleneck_lsu_spec_past_addr_unknown_p1", "LSU", "port 1 loads issued speculatively past unresolved older store addresses"),
    ("xs_bottleneck_lsu_ordering_violations", "LSU", "load/store ordering violations detected by the load queue"),
    ("xs_bottleneck_lsu_spec_replays", "LSU", "ordering violations attributed to unresolved-store speculative loads"),
    ("xs_bottleneck_lsu_replay_valid", "LSU", "replay redirects emitted after LSU ordering violations"),
    ("xs_bottleneck_lsu_spec_killed_by_flush", "LSU", "unresolved-store speculative loads killed by unrelated full flushes"),
    ...
    ("xs_delivery_score_active", "frontend delivery scoreboard", "cycles where the delivery scoreboard tracks an incomplete owner"),
    ("xs_delivery_no_emit", "frontend delivery scoreboard", "data-present no-emit cycles seen by the delivery scoreboard"),
    ("xs_delivery_no_emit_score_active", "frontend delivery scoreboard", "data-present no-emit cycles while an incomplete owner is tracked"),
    ("xs_delivery_no_emit_no_score", "frontend delivery scoreboard", "data-present no-emit cycles with no delivery scoreboard owner"),
    ("xs_delivery_no_emit_owner_match", "frontend delivery scoreboard", "data-present no-emit cycles whose F2 owner matches the scoreboard owner"),
    ("xs_delivery_no_emit_owner_mismatch", "frontend delivery scoreboard", "data-present no-emit cycles whose F2 owner differs from the scoreboard owner"),
    ("xs_delivery_no_emit_expected_pc", "frontend delivery scoreboard", "data-present no-emit cycles where F2 is at the next required PC"),
    ("xs_delivery_no_emit_already_delivered", "frontend delivery scoreboard", "data-present no-emit cycles where F2 is not at the next required PC for the same owner"),
    ("xs_delivery_no_emit_after_complete", "frontend delivery scoreboard", "data-present no-emit cycles after the tracked owner was marked complete"),
    ("xs_delivery_no_emit_dup_expected", "frontend delivery scoreboard", "duplicate-suppressed no-emit cycles that match the next required PC"),
    ("xs_delivery_no_emit_dup_already_delivered", "frontend delivery scoreboard", "duplicate-suppressed no-emit cycles that do not match the next required PC"),
    ("xs_delivery_no_emit_redirect_hold", "frontend delivery scoreboard", "expected-PC no-emit cycles blocked by redirect"),
    ("xs_delivery_no_emit_pkt_not_ready", "frontend delivery scoreboard", "expected-PC no-emit cycles blocked by packet enqueue readiness"),
    ("xs_delivery_no_emit_frontend_hold", "frontend delivery scoreboard", "expected-PC no-emit cycles blocked by frontend hold"),
    ("xs_delivery_no_emit_fe_stall", "frontend delivery scoreboard", "expected-PC no-emit cycles blocked by IFU/frontend stall"),
    ("xs_delivery_no_emit_straddle", "frontend delivery scoreboard", "expected-PC no-emit cycles overlapping straddle advance"),
    ("xs_delivery_no_emit_remainder", "frontend delivery scoreboard", "expected-PC no-emit cycles overlapping remainder handling"),
    ("xs_delivery_no_emit_control", "frontend delivery scoreboard", "expected-PC no-emit cycles with predecoded control in the line"),
    ("xs_delivery_no_emit_taken_control", "frontend delivery scoreboard", "expected-PC no-emit cycles with taken control in the line"),
    ("xs_delivery_no_emit_other", "frontend delivery scoreboard", "expected-PC no-emit cycles not classified by the known hold reasons"),
    ("xs_delivery_cursor_same_line", "frontend delivery scoreboard", "already-delivered no-emit cycles whose next required PC is in the same line"),
    ("xs_delivery_cursor_cross_line", "frontend delivery scoreboard", "already-delivered no-emit cycles whose next required PC is in another line"),
    ("xs_delivery_cursor_seq_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where sequential next PC matches the scoreboard next PC"),
    ("xs_delivery_cursor_dup_next_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where duplicate next PC matches the scoreboard next PC"),
    ("xs_delivery_cursor_req_pc_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where IFU request PC matches the scoreboard next PC"),
    ("xs_delivery_cursor_successor_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where successor request PC matches the scoreboard next PC"),
    ("xs_delivery_cursor_runahead_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where runahead pending PC matches the scoreboard next PC"),
    ("xs_delivery_cursor_bpu_target_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where BPU redirect target matches the scoreboard next PC"),
    ("xs_delivery_cursor_req_redirect", "frontend delivery scoreboard", "already-delivered no-emit cycles overlapping request redirect"),
    ("xs_delivery_cursor_bpu_redirect", "frontend delivery scoreboard", "already-delivered no-emit cycles overlapping BPU redirect"),
    ("xs_delivery_cursor_commit_redirect", "frontend delivery scoreboard", "already-delivered no-emit cycles overlapping commit redirect"),
    ("xs_delivery_cursor_line_state_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where F2 line state already covers the scoreboard next PC"),
    ("xs_delivery_cursor_icq_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where ICQ head covers the scoreboard next PC"),
    ("xs_delivery_cursor_ftq_next_start_match", "frontend delivery scoreboard", "already-delivered no-emit cycles where the next FTQ owner start PC matches the scoreboard next PC"),
    ("xs_post_delivery_dup_base", "frontend delivery scoreboard", "owner-tagged last-emit duplicate cycles whose duplicate and sequential next PCs agree in the same line"),
    ("xs_post_delivery_dup_ready", "frontend delivery scoreboard", "post-delivery duplicate cycles that pass the current cursor-advance safety gates"),
    ("xs_post_delivery_dup_not_delivered", "frontend delivery scoreboard", "post-delivery duplicate cycles where the IFU owner-delivered bit is still clear"),
    ("xs_post_delivery_dup_complete", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked because the owner is complete"),
    ("xs_post_delivery_dup_pred_ctl", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked by predicted-control window safety"),
    ("xs_post_delivery_dup_pred_ctl_taken", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ owner predicts taken"),
    ("xs_post_delivery_dup_pred_ctl_not_taken", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ owner predicts not taken"),
    ("xs_post_delivery_dup_pred_ctl_predecode", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles with a predecoded control in the current packet"),
    ("xs_post_delivery_dup_pred_ctl_bp_taken", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles where the current packet resolves as BPU taken"),
    ("xs_post_delivery_dup_pred_ctl_cond", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ control type is conditional"),
    ("xs_post_delivery_dup_pred_ctl_jal", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ control type is JAL or call"),
    ("xs_post_delivery_dup_pred_ctl_jalr", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ control type is JALR"),
    ("xs_post_delivery_dup_pred_ctl_ret", "frontend delivery scoreboard", "predicted-control blocked post-delivery duplicate cycles whose FTQ control type is return"),
    ("xs_post_delivery_dup_next_owner", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked by next-owner boundary safety"),
    ("xs_post_delivery_dup_remainder", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked by straddle or remainder cursor state"),
    ("xs_post_delivery_dup_redirect", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked by redirect state"),
    ("xs_post_delivery_dup_fe_stall", "frontend delivery scoreboard", "post-delivery duplicate cycles blocked by frontend stall"),
    ("xs_post_delivery_dup_take_next", "frontend delivery scoreboard", "post-delivery duplicate cycles where next-owner takeover has higher priority"),
    ("xs_post_delivery_dup_take_request", "frontend delivery scoreboard", "post-delivery duplicate cycles where request-owner takeover has higher priority"),
    ("xs_post_delivery_dup_take_remainder", "frontend delivery scoreboard", "post-delivery duplicate cycles where remainder request-owner takeover has higher priority"),
    ("xs_post_delivery_dup_other", "frontend delivery scoreboard", "post-delivery duplicate cycles not classified by the known cursor blockers"),
    ("xs_delivery_owner_switch", "frontend delivery invariant", "delivery owner switched before previous owner completed, must remain 0"),
    ("xs_delivery_noncontig_pcs", "frontend delivery invariant", "non-contiguous delivered PC stream, must remain 0"),
