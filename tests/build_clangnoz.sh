...
# suite CFLAGS.  The suite GCC (Xilinx 13.4) accepts -march=... but
...
#   tests/{riscv-bench,embench}/baremetal/<name>-clangnoz.elf
#   tests/hex/<name>-clangnoz.hex
set -euo pipefail
...
OBJ_DIR="$(mktemp -d /tmp/clangnoz_obj.XXXX)"
...
build() {
    ...
    echo "=== Building ${NAME}-clangnoz (clang) ==="
    local OBJS=()
    local i=0
    for s in "${SRCS[@]}"; do
    ...
    local OUT_ELF="$OUT_DIR/${NAME}-clangnoz.elf"
    local OUT_HEX="$ROOT/tests/hex/${NAME}-clangnoz.hex"
    "$CC" "${GCC_LINK_FLAGS[@]}" \
        -T "$CMPRO_BM/link.ld" \
        "$CMPRO_BM/crt0.S" \
        "${OBJS[@]}" \
...
echo "All -clangnoz builds complete."
