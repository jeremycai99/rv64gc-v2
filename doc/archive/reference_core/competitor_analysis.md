...
> MegaBOOM investigation history and append-only provenance; use the unified
...
## Methodology — read before interpreting MegaBOOM data
...
Use the following evidence ladder for the MegaBOOM landscape:

| Tier | Evidence type | Trust level | How to use it |
|---|---|---|---|
| **A** | Same binary, same counter window, in-kernel `mcycle/minstret`, plus per-cycle commit/fetch/LSU counters | Decisive | Use for IPC and pipeline-behavior conclusions. This is not complete yet for BOOM. |
| **B** | Same binary completes on both simulators, but only total Chipyard sim cycles are available | Functional only | Confirms compatibility. Do **not** interpret raw BOOM total cycles as kernel IPC because BootROM, DTB/MMU/PMP setup, cold caches, FastRAM/fesvr idle, and HTIF polling are included. |
| **C** | Source/config audit of MegaBOOM feature flags and structures | Strong directional evidence | Use to identify candidate mechanisms only when they map to a measured rv64gc-v2 bottleneck. |
| **D** | Published SonicBOOM/MegaBOOM scores and papers | Sanity check | Use as a public floor and plausibility bound, not as a directly comparable run unless binary/toolchain/windows match. |
| **E** | Other open RV cores, e.g. XiangShan, NaxRiscv, RSD, OpenC910 | Architectural reference | Use for ideas and instrumentation patterns. Do not treat as direct pass/fail targets for this MegaBOOM comparison. |

Comparison rules:

1. **Separate harness time from core time.** Raw Chipyard Verilator cycles are
   not kernel cycles until BOOM exposes either final `mcycle/minstret` for the
...
3. **Demand pipeline counters, not only scores.** The rv64gc-v2 evidence says
   CoreMark/Dhrystone are dominated by `HEAD_WAIT_BACKLOG`; the BOOM side must
   ...
   closes the gap.
4. **Map each borrowed idea to a measured local bottleneck.** A MegaBOOM feature
   ...
   not.
5. **Use BOOM as the public floor, not the SOTA ceiling.** MegaBOOM is the
   primary calibration target because it is open and runnable here. XiangShan
...
1. **Tier B → Tier A (IPC only).** Patch BOOM/Chipyard to print
   ...
   then, §10 proves BOOM can run the binaries, but it does not provide clean
   BOOM IPC.
2. **Tier A IPC → Tier A pipeline-behavior.** Add BOOM-side per-cycle counter
   ...
   the corpus directory is authoritative). Concretely BOOM must emit, per
   ...
   - **Tool reuse:** the three .py tools above already consume the schema;
     re-run them against BOOM-side traces and the comparison becomes
     side-by-side textual diff per category (PEAK / HEAD_WAIT_BACKLOG /
     FRONTEND_LIMITED / DISPATCH_BLOCKED / FLUSH).
   - **NaxRiscv reference** (Tier E) is already collected as
...
**Last edit:** 2026-05-03 ~00:35 by Codex. Non-MegaBOOM Tier E reference test update added as §12; §12 supersedes the older Tier E pending notes where they conflict.
...
- (Tier E expanded) NaxRiscv same-binary microbench and native benchmark traces collected; RSD native replay/memory-dependence tests collected; XiangShan RTL/Verilator generation completed but local emulator compile is blocked by host Verilator wrapper/header compatibility (§12).
...
## 12. Non-MegaBOOM Tier E reference update (2026-05-03)

This section updates the Tier E landscape after running the other local RV
reference cores. It does not change the MegaBOOM methodology: BOOM remains the
quantitative public floor, and non-MegaBOOM cores remain architectural and
...
correctly classified NaxRiscv/RSD as references rather than apples-to-apples
...
|---|---|---|---|
| Harness/core separation | Mentioned indirectly for BOOM and reference cores. | Hard rule: raw simulator cycles are not kernel IPC unless the benchmark window is instrumented. | Current method is stricter and should supersede. |
| Other cores | Smoke references for trace format/mechanism inspection. | Tier E: architectural reference only; never direct pass/fail for MegaBOOM comparison. | Same direction, now formalized. |
| BOOM feature borrowing | Feature diff was allowed as directional evidence. | Feature must map to measured rv64gc-v2 stall class before RTL action. | Current method blocks premature copying. |
| Quantitative gap claims | Earlier sections contained raw-cycle and residual-gap discussion. | Claims like "BOOM is X% faster" are blocked until BOOM Tier A IPC/counters exist. | Current method is the correct guardrail. |

### 12.2 Results now available

| Core | Tier E status | Result | Use |
|---|---|---|---|
| **NaxRiscv** | Expanded beyond smoke. | Same-binary microbenches pass; same-source CoreMark/Dhrystone attempts trap; native RV64IMAFDC CoreMark/Dhrystone pass. | Use O3Pipe summaries for stage-latency, retire-width, and low-cost replay/load-hit instrumentation ideas. |
| **RSD** | Expanded beyond pending. | Four native RV32 tests pass: `IntRegImm`, `LoadAndStore`, `MemoryDependencyPrediction`, `ReplayQueueTest`. | Use RSD/Kanata logs for replay queue, memory-dependence, and store-load-forwarding behavior. |
| **XiangShan** | Source/config reference; runtime blocked. | `TLMinimalConfig` RTL generation and Verilator C++ generation complete; emulator compile fails before `build/emu`. | Use for source/config audit only until host Verilator wrapper compatibility is resolved. |
...
The non-MegaBOOM reference runs strengthen the methodology, but they do not
authorize a different immediate RTL direction.

What they authorize now:

- Use NaxRiscv O3Pipe summaries to calibrate stage-latency vocabulary and trace
  parsers.
- Use RSD Kanata/RSD logs to inspect replay and memory-dependence flows before
  proposing LSQ policy changes.
- Use XiangShan as a source/config reference for FTQ/FDIP, prefetch, fusion,
  and LSU replay concepts.

What they still block:

- Do not compare Nax/RSD native IPC against rv64gc-v2 signoff IPC.
- Do not claim XiangShan runtime behavior until `build/emu` exists and a
  workload completes.
- Do not copy XiangShan or BOOM structures wholesale. A borrowed mechanism must
  map back to a measured rv64gc-v2 stall class.

The strong design verdict remains:

1. Keep the BOOM TestDriver `mcycle/minstret` patch as the next BOOM-side
   empirical unlock.
2. In rv64gc-v2, the only no-BOOM-dependency RTL action still clearing the
