...
- Build: `./build_dsim.sh` passed after the current `fetch_top.sv` checker
  fix.
- Run artifact:
...
4 in the next_pc priority chain at fetch_top.sv:170). F1 only advances
...
2. **Split the IFU request owner from the IFU-writeback owner.** Done on
   ...
   has been removed; `fetch_top.sv` now asserts request-accept in the normal
...
were generated before the latest `fetch_top.sv` edit or had empty frontend
...
- Rebuild: `./build_dsim.sh` passed after the current `fetch_top.sv` timestamp,
...
`fetch_top.sv` infer correctness from queue-head or locally latched line
...
explicitly, and `fetch_top.sv` uses that line address to fill
...
prior session); SVA invariants (fetch_top.sv) catch pipeline timing
...
| `frontend/ftq/ftq.sv` | `Ftq` | Already separate. Current owner pointers are `alloc_to_ifu`, `ifu_to_wb`, and `ifu_to_commit`. | Keep `ftq_entry_t` and current valid/ready/pop interface; later rename ports toward `BpuToFtq`, `FtqToIfu`, `IfuToFtq` bundles. |
| `frontend/ifu/ifu.sv` | `Ifu` | Current slices own the registered F1 PC/valid sequencer, conservative request PC/stall/FTQ enqueue boundary, IFU request-pop fire, last-allocation anti-dup state, straddle-remainder cursor, completion owner-live matching against the FTQ IFU-writeback owner, IFU completion-side delivery push, FTQ IFU-pop, the IFU work cursor, and owner handoff policy. Decode/commit-side FTQ pop still remains in `fetch_top.sv`. | Redirect and F1 advance inputs, `FtqToIfu` owner view, request-owner metadata inputs, ICQ/IBuffer/FTQ readiness inputs, packet/owner-complete inputs, `consumed_remainder_o` and `owner_live_o` for existing probes, and `f2_work_*` outputs to line fetch, packet build, and debug/probe logic. |
| `frontend/ifu/ifu_duplicate_guard.sv` | IFU local helper | Current slice owns the legacy duplicate/replay suppression registers that were embedded in `fetch_top.sv`. This is a behavior-preserving quarantine only; the intended architecture still removes the need for this guard after FTQ/IFU/IBuffer ownership prevents duplicate packet production structurally. | Current F2 work identity, emit event, branch/sequence next-PC sources, and debug/probe outputs for duplicate attribution. |
| `frontend/ifu/ifu_line_fetch.sv`, current leaf `frontend/ifu/next_line_prefetch_buffer.sv` | `Ifu` + `ICache` adapter | Current slice owns the I-cache, NLPB adapter, request-owner retime, ICQ response association, line-qualified ICQ pop/stale drain, IFU-writeback owner-match observation, and same-line line-state reuse. | `IfuToICache`: request valid/addr. `ICacheToIfu`: accepted line data plus ICQ head metadata and FTQ identity. Keep `icache.sv` separate. |
| `frontend/pred/pred_checker.sv` | `PredChecker` | FTQ predicted-control matching, static CFI override, branch target choice, subgroup split choice, final count, owner completion, redirect/RAS/GHR requests around lines 1763-2383. | Inputs: extracted slots, predecode result, FTQ owner entry, BPU prediction metadata. Outputs: redirect request/target, final count, owner complete, RAS op, TAGE spec update. |
| `frontend/instr/instr_boundary.sv` | `InstrBoundary` | Raw parcel extraction, RVC length detection, straddle detect, and remainder state around lines 1472-1626 and 2385-2419. | Inputs: work PC, 512-bit line, valid, redirect/stall. Outputs: raw slot PCs/instructions/RVC flags, extract count, straddle/remainder consume, seq helper signals. |
| `frontend/instr/predecode.sv` | `PreDecode` / `F3PreDecode` | Earliest and second control-flow decode around lines 1648-1761; helper functions `is_link_reg`, `imm_b64`, `imm_j64`. | Inputs: raw/decompressed slots, slot PCs/valid/RVC, extract count, RAS top. Outputs: first/second CFI metadata and owner conditional prediction flag. |
| `frontend/instr/rvc_expander.sv` | `RvcExpander` | Generate block of `rvc_decompress` instances around lines 1628-1646. | Inputs: raw halfword array. Outputs: decompressed word/is_rvc/illegal arrays. |
...
5. Keep debug probes and SVA with the owner module they observe as state moves.
   Until then, leave them in `fetch_top.sv` so hierarchical testbench probes
   continue to work.
...
|---|---|---|---|
| `predecode.sv` extraction | Moved first/second CFI decode and helper immediate/link-register functions out of `fetch_top.sv`; `fetch_top.sv` now instantiates the leaf module. | `benchmark_results/20260506_predecode_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner/stale counters remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `instr_boundary.sv` extraction | Moved raw slot extraction, RVC length detection, straddle detection, and cross-line remainder state out of `fetch_top.sv`; old signal names remain visible as module outputs for existing probes. | `benchmark_results/20260506_instr_boundary_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner/stale counters remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `instr_boundary.sv` sequential PC | Moved F2 sequential next-PC and seq-valid generation from `fetch_top.sv` into `frontend/instr/instr_boundary.sv`, next to slot extraction, straddle detection, and remainder handoff. | `benchmark_results/20260506_instr_boundary_seq_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral instruction-boundary ownership cleanup; no performance claim. |
| `rvc_expander.sv` extraction | Moved the multi-slot `rvc_decompress` generate wrapper out of `fetch_top.sv`; `rvc_decompress.sv` remains the decompressor leaf. | `benchmark_results/20260506_rvc_expander_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `instr_compact.sv` extraction | Moved mechanical fetch-packet assembly out of `fetch_top.sv`; prediction, owner-completion, RAS, and GHR decisions intentionally stayed in `fetch_top.sv` until the following `pred_checker.sv` slice. | `benchmark_results/20260506_instr_compact_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner/stale counters remained zero; owner switch and non-contiguous packet checks remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `instr_compact.sv` emit control | Moved payload-valid, packet emit, and PC-consumed generation from `fetch_top.sv` into `frontend/instr/instr_compact.sv`. The helper now combines work/data validity, duplicate suppression, IBuffer enqueue readiness, and line-straddle advancement while preserving the top-level `f2_has_emit_payload_c`, `f2_will_emit_c`, and `f2_pc_consumed_c` observation wires. | `benchmark_results/20260507_instr_compact_emit_control_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral packet-formation ownership cleanup; no performance claim. |
| `pred_checker.sv` extraction | Moved predicted-control validation, static control override, subgroup split decision, final count, owner-complete boolean, BPU redirect request, RAS action, and speculative GHR action into `frontend/pred/pred_checker.sv`; subgroup seed registers remain in `fetch_top.sv` for this behavior-neutral slice. | `benchmark_results/20260506_pred_checker_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, and owner/stale counters remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `pred_checker.sv` subgroup seed state | Moved the registered subgroup seed state from `fetch_top.sv` into `frontend/pred/pred_checker.sv`. `fetch_top.sv` now observes the seed hit and seed metadata outputs while the prediction checker owns load, redirect clear, and request-consume clear policy. | `benchmark_results/20260506_pred_checker_seed_state_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral prediction ownership cleanup; no performance claim. |
| `ibuffer.sv` wrapper | Added `frontend/ibuffer/ibuffer.sv` as the architectural decode-facing packet buffer wrapper around the existing `fetch_packet_buffer.sv` FIFO, and changed `fetch_top.sv` to instantiate `ibuffer`. | `benchmark_results/20260506_ibuffer_wrapper_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, and owner/stale counters remained zero. | Accepted as behavior-neutral target-shape wrapper; no performance claim. |
| `ifu_line_fetch.sv` adapter | Added `frontend/ifu/ifu_line_fetch.sv` as the line-fetch adapter around `icache.sv` and `next_line_prefetch_buffer.sv`; `fetch_top.sv` now consumes its merged line-response stream. ICQ and same-line line-state policy remain in `fetch_top.sv` for the next stateful IFU slice. The trace harness debug hierarchy was updated from `u_fetch_top.u_icache` to `u_fetch_top.u_ifu_line_fetch.u_icache`. | `benchmark_results/20260506_ifu_line_fetch_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral line-fetch adapter extraction; no performance claim. |
| `ifu_line_fetch.sv` ICQ association | Moved the request-owner retime registers and `icache_resp_queue` instance into `frontend/ifu/ifu_line_fetch.sv`. The module now exposes the ICQ head fields and occupancy to `fetch_top.sv`; line match/stale policy, same-line line-state reuse, and IFU work-cursor updates remain in `fetch_top.sv`. | `benchmark_results/20260506_ifu_line_fetch_icq_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral ICQ ownership extraction; no performance claim. |
| `ifu_line_fetch.sv` line-state reuse | Moved line-qualified ICQ pop/stale drain, accepted-response generation, same-line line-state register, and final F2 line-data selection into `frontend/ifu/ifu_line_fetch.sv`. `fetch_top.sv` now consumes accepted line data, line-reuse state, and ICQ head metadata while the IFU work cursor remains local. | `benchmark_results/20260506_ifu_line_fetch_line_state_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral line-state ownership extraction; no performance claim. |
| `ifu_line_fetch.sv` owner-match observation | Moved the ICQ-head to FTQ IFU-writeback owner-match comparison from `fetch_top.sv` into `frontend/ifu/ifu_line_fetch.sv`, next to ICQ response association and stale-epoch handling. `fetch_top.sv` still exposes `icq_deq_owner_match_c` as a wire for existing simulation assertions. | `benchmark_results/20260507_ifu_line_fetch_owner_match_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral line-fetch ownership cleanup; no performance claim. |
| `ifu.sv` cursor extraction | Added `frontend/ifu/ifu.sv` and moved the registered IFU work cursor plus owner handoff policy out of `fetch_top.sv`. The integration layer still owns F1 PC generation, duplicate suppression, FTQ completion/pop policy, BPU, and packet construction, but consumes the same `f2_work_*` aliases from the IFU module. | `benchmark_results/20260506_ifu_cursor_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral IFU cursor extraction; no performance claim. |
| `ifu.sv` F1 sequencing | Moved the registered F1 PC/valid sequencer out of `fetch_top.sv` and into `frontend/ifu/ifu.sv`. The same redirect, BPU redirect, duplicate-suppression, sequential-consume, and post-remainder priority is preserved; this does not enable proactive runahead. | `benchmark_results/20260506_ifu_f1_sequence_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral F1 sequencing ownership extraction; no performance claim. |
| `ifu.sv` request boundary | Moved request PC selection, frontend stall, I-cache request valid/address, FTQ enqueue valid, and IFU request-pop fire into `frontend/ifu/ifu.sv`. The same conservative lockstep allocation conditions are preserved, including FTQ enqueue readiness, ICQ capacity, IBuffer capacity, last-allocation anti-dup, remainder hold, and same-owner continuation hold. | `benchmark_results/20260506_ifu_request_boundary_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral IFU request-boundary extraction; no performance claim. |
| `bpu.sv` wrapper | Added `src/rtl/core/bpu/bpu.sv` and moved the direct BTB, TAGE-SC-L, and RAS instances out of `fetch_top.sv`. The wrapper owns predictor lookup/update/restore wiring and speculative GHR routing; `fetch_top.sv` still consumes raw BPU outputs to assemble FTQ prediction metadata. | `benchmark_results/20260506_bpu_wrapper_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral BPU wrapper extraction; no performance claim. |
| BPU-to-FTQ request adapter | Moved request-time predicted-control assembly, aux prediction observation, and FTQ entry construction from `fetch_top.sv` into `core/bpu/bpu.sv`. `fetch_top.sv` now receives the assembled `ftq_entry_t` for FTQ enqueue. | `benchmark_results/20260506_bpu_ftq_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral BPU ownership cleanup; no performance claim. |
| BPU F2 prediction retime | Moved the registered F1 to F2 BTB/TAGE prediction snapshot and GHR hold policy from `fetch_top.sv` into `core/bpu/bpu.sv`. `fetch_top.sv` consumes registered F2 predictor metadata from the BPU for `pred_checker.sv`, `instr_compact.sv`, and trace probes. | `benchmark_results/20260507_bpu_f2_retime_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. `FETCH DELIVERY CHECK` and `FETCH OWNER CHECK` violation counters stayed zero; no `INVARIANT_*` errors were reported. `ftq_occ_max=1`, `packet_buf_occ_max=0`, and `xs_f2_owner_idx_mismatch=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral BPU ownership cleanup; no performance claim. |
| BPU GHR hold policy operands | Removed the residual `f2_bpu_ghr_hold_c` policy boolean from `fetch_top.sv`. `core/bpu/bpu.sv` now derives the F2 GHR snapshot hold internally from line-straddle advance, remainder consume, and consumed-remainder source events. | `benchmark_results/20260507_bpu_ghr_hold_policy_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral BPU ownership cleanup; no performance claim. |
| `ifu.sv` request bookkeeping | Moved the last-allocation anti-dup register and consumed-remainder/post-remainder cursor from `fetch_top.sv` into `frontend/ifu/ifu.sv`. `fetch_top.sv` now exposes `consumed_remainder_r` as a wire from the IFU only to preserve existing simulation binds and hierarchy probes. | `benchmark_results/20260507_ifu_bookkeeping_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. `FETCH DELIVERY CHECK` and `FETCH OWNER CHECK` violation counters stayed zero; no `INVARIANT_*` errors were reported. `ftq_occ_max=1`, `packet_buf_occ_max=0`, `xs_f2_owner_idx_mismatch=0`, and stale-owner counters remained zero. | Accepted as behavior-neutral IFU ownership cleanup; no performance claim. |
| `ifu.sv` owner-live comparison | Moved `f2_ftq_owner_live_c` computation from `fetch_top.sv` into `frontend/ifu/ifu.sv`, where it gates owner delivery and FTQ IFU-pop. `fetch_top.sv` still exposes the same signal name as an IFU output wire for existing checkers and traces. | `benchmark_results/20260507_ifu_owner_live_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. `FETCH DELIVERY CHECK` and `FETCH OWNER CHECK` violation counters stayed zero; no `INVARIANT_*` errors were reported. `ftq_occ_max=1`, `packet_buf_occ_max=0`, and `xs_f2_owner_idx_mismatch=0`. | Accepted as behavior-neutral IFU ownership cleanup; no performance claim. |
| `ifu.sv` completion/pop boundary | Moved redirect-without-successor classification, IFU owner-completion candidate, first-packet delivery push, and FTQ IFU-pop generation into `frontend/ifu/ifu.sv`. The decode/commit-side FTQ pop remains in `fetch_top.sv` because it is driven by the IBuffer dequeue owner-complete boundary. | `benchmark_results/20260506_ifu_completion_pop_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral IFU completion/pop extraction; no performance claim. |
| `ifu_duplicate_guard.sv` extraction | Added `frontend/ifu/ifu_duplicate_guard.sv` and moved the legacy last-emitted-PC and redirect replay-block suppression registers out of `fetch_top.sv`. Existing debug/probe signals remain available as module outputs. | `benchmark_results/20260506_ifu_duplicate_guard_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral legacy-guard extraction; no performance claim. |
| `fetch_top.sv` direct top | Renamed the integration top to `frontend/top/fetch_top.sv`, removed the temporary compatibility wrapper, instantiated `fetch_top u_fetch_top` directly from `rv64gc_core_top.sv`, and retargeted testbench hierarchy probes to `u_core.u_fetch_top`. `frontend/ifu/ifu.sv` remains the instruction fetch unit block. | `benchmark_results/20260506_fetch_top_direct_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as direct top naming cleanup; no behavior or performance claim. |
| `fetch_top.sv` alias cleanup | Removed the final compatibility aliases from `fetch_top.sv`; simulation binds now connect directly to `req_ftq_entry_c` and `ftq_ifu_pop_valid`. Final audit shows `frontend/top/` contains only `fetch_top.sv`, with no `always_*`, local `assign`, initial/final block, or simulation task in the integration top. | `benchmark_results/20260507_fetch_top_alias_cleanup_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. XSim fallback also passed `rv64ui_minimal` at cycle 53 with strict owner/delivery violations at zero. | Accepted as final behavior-neutral top-wrapper cleanup; no performance claim. |
| `fetch_delivery_checker.sv` sim bind | Moved the strict fetch delivery stream checker from `fetch_top.sv` into `src/rtl/sim/fetch_delivery_checker.sv` and bound it to `fetch_top` in simulation. `fetch_top.sv` no longer owns the delivery checker `$display`, `$fatal`, or plusarg logic. | `benchmark_results/20260507_fetch_delivery_checker_bind_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. The bound checker still prints `FETCH DELIVERY CHECK` summaries with owner switch and non-contiguous packet counts at zero; owner/stale counters remained zero. | Accepted as behavior-neutral simulation-boundary cleanup; no performance claim. |
| `fetch_owner_checker.sv` sim bind | Moved the same-line owner contract checker from `fetch_top.sv` into `src/rtl/sim/fetch_owner_checker.sv` and bound it to `fetch_top` in simulation. `fetch_top.sv` no longer owns the owner-checker `$display`, `$fatal`, or plusarg logic. | `benchmark_results/20260507_fetch_owner_checker_bind_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. `FETCH OWNER CHECK` reported packet line metadata mismatch, duplicate/replayed PCs, and skipped PCs at zero; `FETCH DELIVERY CHECK` owner switch and non-contiguous packet counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral simulation-boundary cleanup; no performance claim. |
| `fetch_frontend_profiler.sv` sim bind | Moved the XS frontend catch-up/profile counter block from `fetch_top.sv` into `src/rtl/sim/fetch_frontend_profiler.sv` and bound it to `fetch_top` in simulation. The `PERF_PROFILE`, `STAT_DUMP`, and `TRACE_XS_CATCHUP` output text is intentionally unchanged for the benchmark parser. | `benchmark_results/20260507_fetch_frontend_profiler_bind_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. The profile summary still reports nonzero frontend counters; owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral simulation-boundary cleanup; no performance claim. |
| `fetch_trace_probe.sv` sim bind | Moved optional fetch path traces from `fetch_top.sv` into `src/rtl/sim/fetch_trace_probe.sv` and bound it to `fetch_top` in simulation. The `TRACE_FETCH`, `TRACE_FETCH_SPLIT`, `FETCH_DUP_TRACE`, and `TRACE_FETCH_OWNER` plusargs and message text are intentionally unchanged. | `benchmark_results/20260507_fetch_trace_probe_bind_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. The normal profile summary is unchanged; owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. | Accepted as behavior-neutral simulation-boundary cleanup; no performance claim. |
| `fetch_frontend_assertions.sv` sim bind | Moved the frontend SVA invariant block from `fetch_top.sv` into `src/rtl/sim/fetch_frontend_assertions.sv` and bound it to `fetch_top` in simulation. The same invariant names and `$error` text are preserved. | `benchmark_results/20260507_fetch_frontend_assertions_bind_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. No `INVARIANT_*` errors were reported; owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. | Accepted as behavior-neutral simulation-boundary cleanup; no performance claim. |
| Subgroup split knob removal | Removed old simulation plusarg controls for disabling subgroup-split behavior from `fetch_top.sv` and hardwired the production defaults: second-control split enabled, any-second-control split enabled, owner-conditional split enabled, and slot-3 FTQ-taken-only mode disabled. | `benchmark_results/20260507_subgroup_knob_removal_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. No `INVARIANT_*` errors were reported; profile counters and strict owner/delivery counts stayed unchanged. | Accepted as behavior-neutral removal of stale DSE control surface; no performance claim. |
| `pred_checker.sv` subgroup defaults | Moved the fixed production subgroup-split defaults from `fetch_top.sv` wiring into `frontend/pred/pred_checker.sv`, so the prediction checker owns this policy rather than receiving constant integration-layer controls. | `benchmark_results/20260507_pred_checker_subgroup_defaults_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral prediction-checker ownership cleanup; no performance claim. |
| `pred_checker.sv` same-owner continuation | Moved same-cache-line straight-line same-owner continuation classification from `fetch_top.sv` into `frontend/pred/pred_checker.sv`, next to final packet count, subgroup split, and owner-complete classification. The IFU now consumes this checker-owned boundary signal. | `benchmark_results/20260507_pred_checker_same_owner_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral prediction-checker ownership cleanup; no performance claim. |
| `pred_checker.sv` BPU redirect fire | Moved stall-qualified BPU redirect fire generation from `fetch_top.sv` into `frontend/pred/pred_checker.sv`. `fetch_top.sv` keeps the raw redirect request for the IFU request policy and wires the checker-owned fire signal to IFU, BPU retime, instruction boundary, duplicate guard, and trace probes. | `benchmark_results/20260507_pred_checker_bpu_redirect_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral prediction-checker ownership cleanup; no performance claim. |
| IBuffer decode adapter | Moved packet-to-decode output mapping from `fetch_top.sv` into `frontend/ibuffer/ibuffer.sv`. The IBuffer now exposes the monitor-facing output packet and decode-facing fetch signals while `fetch_top.sv` only wires them through. | `benchmark_results/20260506_ibuffer_decode_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral IBuffer ownership cleanup; no performance claim. |
| IBuffer flow-through adapter | Moved IBuffer flow-through observation signals and decode/commit-side FTQ pop generation from `fetch_top.sv` into `frontend/ibuffer/ibuffer.sv`. `fetch_top.sv` keeps the existing wire names for profiler and delivery-checker binds. | `benchmark_results/20260507_ibuffer_flowthrough_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. `FETCH DELIVERY CHECK` and `FETCH OWNER CHECK` violation counters stayed zero; no `INVARIANT_*` errors were reported. `ftq_occ_max=1`, `packet_buf_occ_max=0`, and `xs_f2_owner_idx_mismatch=0`. | Accepted as behavior-neutral IBuffer ownership cleanup; no performance claim. |
| IBuffer dequeue-ready adapter | Moved decode dequeue readiness from `fetch_top.sv` into `frontend/ibuffer/ibuffer.sv`. The IBuffer now derives `deq_ready` from backend stall and frontend hold, while `fetch_top.sv` only observes the resulting signal for FTQ wiring and simulation binds. | `benchmark_results/20260507_ibuffer_deq_ready_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with `+FETCH_DELIVERY_CHECK +FETCH_DELIVERY_STRICT +FETCH_OWNER_CHECK +FETCH_OWNER_STRICT +PERF_PROFILE +PERF_COUNTERS +STAT_DUMP`. Owner switch, non-contiguous packet, duplicate/replayed PC, skipped PC, stale-owner, and `xs_f2_owner_idx_mismatch` counts remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`. | Accepted as behavior-neutral IBuffer ownership cleanup; no performance claim. |
...
| UOP cache folder split | Moved `uop_cache.sv`, `uop_cache_tag_ram.sv`, and `uop_cache_data_ram.sv` from `src/rtl/core/fetch/` to `src/rtl/core/uop_cache/`; the core-top `u_uop_cache` instance and Stage 1 disabled/zero-activity contract are unchanged. | `benchmark_results/20260506_uop_cache_folder_split_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner/stale counters remained zero; legacy loop-buffer and standalone decoded-op replay activity remained zero. | Accepted as physical RTL ownership cleanup; no behavior or performance claim. |
| Frontend ownership folder split | Removed the catch-all `src/rtl/core/fetch/` RTL folder. Moved integration to `frontend/top/`, FTQ to `frontend/ftq/`, line-fetch helper to `frontend/ifu/`, packet buffer to `frontend/ibuffer/`, and RVC/boundary/predecode helpers to `frontend/instr/`. | `benchmark_results/20260506_frontend_folder_split_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner/stale counters remained zero; legacy loop-buffer and standalone decoded-op replay activity remained zero. | Accepted as systematic physical RTL ownership cleanup; no behavior or performance claim. |
...
| SVA invariants (`src/rtl/sim/fetch_frontend_assertions.sv`) | d16d095 | Pipeline timing assertions bound to `fetch_top` in simulation |
...
| IFU work cursor (`ifu_work_item_t`) | working tree 2026-05-07 | Stateful PC/FTQ/line work item; raw F2 PC/FTQ mirror registers retired; IFU owns last-allocation anti-dup and straddle-remainder cursor state; clean owner handoff uses FTQ next-owner identity plus cursor `f2_seq_next_pc`; redirect handoff can use the matching FTQ next-owner view; line gates use active `line_addr`, and extraction/owner/completion/debug probes use `f2_work_*` aliases |
