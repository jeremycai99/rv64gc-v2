...
- **Prediction checker boundary:** `frontend/pred/pred_checker.sv` owns
  ...
  classification, RAS/GHR action requests, and the registered subgroup seed
  state used to carry branch-owner prediction metadata into the following
  request. The production subgroup-split defaults are owned inside
  `pred_checker.sv`; `fetch_top.sv` no longer wires fixed production policy
  controls through the integration layer. Old simulation plusarg controls for
  disabling this behavior have been retired from the core.
