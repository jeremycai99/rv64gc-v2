...
| `frontend/ifu/ifu.sv` | `Ifu` | Current slices own the registered F1 PC/valid sequencer, conservative request PC/stall/FTQ enqueue boundary, IFU request-pop fire, IFU completion-side delivery push, FTQ IFU-pop, the IFU work cursor, and owner handoff policy. Decode/commit-side FTQ pop still remains in `fetch_top.sv`. | Redirect and F1 advance inputs, `FtqToIfu` owner view, request-owner metadata inputs, remainder inputs, ICQ/IBuffer/FTQ readiness inputs, packet/owner-complete inputs, and `f2_work_*` outputs to line fetch, packet build, and debug/probe logic. |
