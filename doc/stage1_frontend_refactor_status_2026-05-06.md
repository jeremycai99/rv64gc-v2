...
| `bpu/bpu.sv` | `Bpu` | Current slice owns the BTB, TAGE-SC-L, and RAS leaf instances plus lookup/update/restore wiring, speculative GHR routing, and request-time FTQ prediction assembly. The predictor leaves live in `src/rtl/core/bpu/`, not `src/rtl/core/fetch/`. | `lookup_pc`, `aux_lookup_pc`, commit/update/restore inputs, raw BTB/TAGE/RAS prediction outputs, RAS snapshot, `ghr_out`, speculative update request, and assembled `ftq_entry_t` request metadata. |
