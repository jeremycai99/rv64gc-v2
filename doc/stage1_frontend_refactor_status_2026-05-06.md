...
| `ifu.sv` | `Ifu` | Current slices own the registered F1 PC/valid sequencer, conservative request PC/stall/FTQ enqueue boundary, IFU request-pop fire, the IFU work cursor, and owner handoff policy. Duplicate/replay guard and FTQ completion/commit-pop policy still remain in `fetch_unit.sv`. | Redirect and F1 advance inputs, `FtqToIfu` owner view, request-owner metadata inputs, remainder inputs, ICQ/IBuffer/FTQ readiness inputs, and `f2_work_*` outputs to line fetch, packet build, and debug/probe logic. |
