...
| `frontend/ifu/ifu_line_fetch.sv`, current leaf `frontend/ifu/next_line_prefetch_buffer.sv` | `Ifu` + `ICache` adapter | Current slice owns the I-cache, NLPB adapter, request-owner retime, and ICQ response association. IFU work cursor and same-line line-state reuse still remain in `fetch_unit.sv` around lines 207-759. | `IfuToICache`: request valid/addr. `ICacheToIfu`: ICQ head valid/hit/data plus request PC and FTQ identity. Keep `icache.sv` separate. |
