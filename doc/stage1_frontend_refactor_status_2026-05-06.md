...
| `bpu/bpu.sv` | `Bpu` | Current slice owns the BTB, TAGE-SC-L, and RAS leaf instances plus lookup/update/restore wiring and speculative GHR routing. Request-time FTQ prediction assembly still remains in `fetch_top.sv` until the final top split. The predictor leaves live in `src/rtl/core/bpu/`, not `src/rtl/core/fetch/`. | `lookup_pc`, `aux_lookup_pc`, commit/update/restore inputs, raw BTB/TAGE/RAS prediction outputs, RAS snapshot, `ghr_out`, and speculative update request. |
