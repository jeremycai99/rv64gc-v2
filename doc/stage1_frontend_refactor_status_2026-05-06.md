...
|---|---|---|---|
| `frontend/top/fetch_unit.sv`, later `fetch_top.sv` | `FrontendInlinedImp` | Current `fetch_unit.sv` top-level ports and pure wiring around FTQ/BPU/IFU/ICache/IBuffer. | Preserve the existing `fetch_unit` external port contract initially; later `fetch_unit.sv` can become a compatibility wrapper around `fetch_top`. |
| `bpu/bpu.sv` | `Bpu` | Current slice owns the BTB, TAGE-SC-L, and RAS leaf instances plus lookup/update/restore wiring and speculative GHR routing. Request-time FTQ prediction assembly still remains in `fetch_unit.sv` until the final top split. The predictor leaves live in `src/rtl/core/bpu/`, not `src/rtl/core/fetch/`. | `lookup_pc`, `aux_lookup_pc`, commit/update/restore inputs, raw BTB/TAGE/RAS prediction outputs, RAS snapshot, `ghr_out`, and speculative update request. |
...
| `instr_compact.sv` extraction | Moved mechanical fetch-packet assembly out of `fetch_unit.sv`; prediction, owner-completion, RAS, and GHR decisions intentionally stayed in `fetch_unit.sv` until the following `pred_checker.sv` slice. | `benchmark_results/20260506_instr_compact_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner/stale counters remained zero; owner switch and non-contiguous packet checks remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `pred_checker.sv` extraction | Moved predicted-control validation, static control override, subgroup split decision, final count, owner-complete boolean, BPU redirect request, RAS action, and speculative GHR action into `frontend/pred/pred_checker.sv`; subgroup seed registers remain in `fetch_unit.sv` for this behavior-neutral slice. | `benchmark_results/20260506_pred_checker_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, and owner/stale counters remained zero. | Accepted as behavior-neutral structural cleanup; no performance claim. |
| `ibuffer.sv` wrapper | Added `frontend/ibuffer/ibuffer.sv` as the architectural decode-facing packet buffer wrapper around the existing `fetch_packet_buffer.sv` FIFO, and changed `fetch_unit.sv` to instantiate `ibuffer`. | `benchmark_results/20260506_ibuffer_wrapper_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, and owner/stale counters remained zero. | Accepted as behavior-neutral target-shape wrapper; no performance claim. |
| `ifu_line_fetch.sv` adapter | Added `frontend/ifu/ifu_line_fetch.sv` as the line-fetch adapter around `icache.sv` and `next_line_prefetch_buffer.sv`; `fetch_unit.sv` now consumes its merged line-response stream. ICQ and same-line line-state policy remain in `fetch_unit.sv` for the next stateful IFU slice. The trace harness debug hierarchy was updated from `u_fetch_unit.u_icache` to `u_fetch_unit.u_ifu_line_fetch.u_icache`. | `benchmark_results/20260506_ifu_line_fetch_adapter_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral line-fetch adapter extraction; no performance claim. |
| `ifu_line_fetch.sv` ICQ association | Moved the request-owner retime registers and `icache_resp_queue` instance into `frontend/ifu/ifu_line_fetch.sv`. The module now exposes the ICQ head fields and occupancy to `fetch_unit.sv`; line match/stale policy, same-line line-state reuse, and IFU work-cursor updates remain in `fetch_unit.sv`. | `benchmark_results/20260506_ifu_line_fetch_icq_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral ICQ ownership extraction; no performance claim. |
| `ifu_line_fetch.sv` line-state reuse | Moved line-qualified ICQ pop/stale drain, accepted-response generation, same-line line-state register, and final F2 line-data selection into `frontend/ifu/ifu_line_fetch.sv`. `fetch_unit.sv` now consumes accepted line data, line-reuse state, and ICQ head metadata while the IFU work cursor remains local. | `benchmark_results/20260506_ifu_line_fetch_line_state_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral line-state ownership extraction; no performance claim. |
| `ifu.sv` cursor extraction | Added `frontend/ifu/ifu.sv` and moved the registered IFU work cursor plus owner handoff policy out of `fetch_unit.sv`. The integration layer still owns F1 PC generation, duplicate suppression, FTQ completion/pop policy, BPU, and packet construction, but consumes the same `f2_work_*` aliases from the IFU module. | `benchmark_results/20260506_ifu_cursor_extraction_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, and packet-stale counters remained zero. | Accepted as behavior-neutral IFU cursor extraction; no performance claim. |
| `ifu.sv` F1 sequencing | Moved the registered F1 PC/valid sequencer out of `fetch_unit.sv` and into `frontend/ifu/ifu.sv`. The same redirect, BPU redirect, duplicate-suppression, sequential-consume, and post-remainder priority is preserved; this does not enable proactive runahead. | `benchmark_results/20260506_ifu_f1_sequence_smoke`: Dhrystone 100 PASS (`mcycle=27598`, `minstret=49088`), CoreMark 1 PASS (`mcycle=219901`, `minstret=332154`) with the same strict owner/delivery/perf plusargs. Owner switch, non-contiguous packet, packet-line mismatch, `xs_f2_owner_idx_mismatch`, packet-stale counters, and stale-owner counters remained zero. `ftq_occ_max=1` and `packet_buf_occ_max=0`, as expected for behavior-neutral cleanup. | Accepted as behavior-neutral F1 sequencing ownership extraction; no performance claim. |
