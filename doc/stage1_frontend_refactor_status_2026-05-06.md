...
| `frontend/ifu/ifu_line_fetch.sv`, current leaf `frontend/ifu/next_line_prefetch_buffer.sv` | `Ifu` + `ICache` adapter | ICache request/response association, NLPB, ICQ, and same-line line-state reuse around lines 207-759. | `IfuToICache`: request valid/addr. `ICacheToIfu`: response valid/hit/data plus request PC and FTQ identity. Keep `icache.sv` separate. |
