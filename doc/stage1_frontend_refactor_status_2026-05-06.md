...
| `ifu.sv` | `Ifu` | First slice now owns the registered IFU work cursor and owner handoff policy. F1 PC generation, request ready/alloc policy, duplicate/replay guard, and FTQ completion/commit-pop policy still remain in `fetch_unit.sv`. | `FtqToIfu` owner view, redirect/remainder inputs, request-owner inputs, and `f2_work_*` outputs to line fetch, packet build, and debug/probe logic. |
