...
| `frontend/ifu/ifu_line_fetch.sv`, current leaf `frontend/ifu/next_line_prefetch_buffer.sv` | `Ifu` + `ICache` adapter | First slice now owns the I-cache plus NLPB adapter. ICQ request/response association and same-line line-state reuse still remain in `fetch_unit.sv` around lines 207-759. | `IfuToICache`: request valid/addr. `ICacheToIfu`: response valid/hit/data plus request PC and FTQ identity. Keep `icache.sv` separate. |
