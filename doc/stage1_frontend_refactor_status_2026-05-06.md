...
| `ifu.sv` | `Ifu` | Current slices own the registered F1 PC/valid sequencer, the IFU work cursor, and owner handoff policy. Request ready/alloc policy, duplicate/replay guard, and FTQ completion/commit-pop policy still remain in `fetch_unit.sv`. | Redirect and F1 advance inputs, `FtqToIfu` owner view, request-owner inputs, remainder inputs, and `f2_work_*` outputs to line fetch, packet build, and debug/probe logic. |
